...
if [ -e /data/raft/raft.db ]; then
	...
	exec consul agent -server -ui -config-dir=/etc/consul/ -datacenter="$CONSUL_DC_NAME" -retry-join="$CONSUL_DNS_NAME" -rejoin
else
	if [ "$CONSUL_DC_NAME" ] && [ "$CONSUL_ENCRYPT_TOKEN" ] && [ "$CONSUL_CLUSTER_SIZE" ] && [ "$CONSUL_DNS_NAME" ]; then
		...
		exec consul agent -server -ui -config-dir=/etc/consul/ -datacenter="$CONSUL_DC_NAME" -bootstrap-expect="$CONSUL_CLUSTER_SIZE" -retry-join="${CONSUL_BOOTSTRAP_HOST:-127.0.0.1}" -retry-join="$CONSUL_DNS_NAME" 
-encrypt="$CONSUL_ENCRYPT_TOKEN"
		...
		printf "The following environment variables are used to configure the cluster (if provided):\nCONSUL_ACL_MASTER_TOKEN: Gets assigned to the 
acl_master_token configuration stanza\nCONSUL_ACL_DC: The Consul ACL Datacenter, defaults to the value of CONSUL_DC_NAME if not provided\nCONSUL_BOOTSTRAP_HOST: 
A Consul server node to join on startup, only used the first time the container runs. Defaults to 127.0.0.1\nCONSUL_ENVIRONMENT: if set to 'prod', it will 
configure 
Consul for high 
performance. If it is not set and you are running on AWS EC2 or Google Cloud Engine (GCE), a performance setting will be set according to your instance type on the first run of the container\n\n" >&2
