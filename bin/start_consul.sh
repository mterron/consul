...
if [ -e /data/raft/raft.db ]; then
	...
	if [ "$CONSUL_DC_NAME" ] && [ "$CONSUL_ENCRYPT_TOKEN" ] && [ "$CONSUL_CLUSTER_SIZE" ] && [ "$CONSUL_BOOTSTRAP_HOST" ] && [ "$CONSUL_DNS_NAME" ]; then
		...
		if [ -z "$CONSUL_ACL_MASTER_TOKEN" ]; then
			...
			log "acl_master_token is: $ACL_MASTER_TOKEN, please set this environment variable before starting the rest of the Consul server nodes" 
		fi
		...
		printf "Consul-betterscratch container\nUsage\n-----\n" >&2
