...
if [ ! "${CONSUL_ENVIRONMENT:-dev}" = 'prod' ]; then
	...
	if [ -f /sys/hypervisor/uuid ] && [ "$(head -c 3 /sys/hypervisor/uuid)" = 'ec2' ]; then
		...
		case "$EC2_INSTANCE_SIZE" in
			nano)   su -s/bin/sh consul -c "{ rm /etc/consul/consul.json; jq '.performance.raft_multiplier = 5' > /etc/consul/consul.json; } < /etc/consul/consul.json" ;;
			micro)  su -s/bin/sh consul -c "{ rm /etc/consul/consul.json; jq '.performance.raft_multiplier = 4' > /etc/consul/consul.json; } < /etc/consul/consul.json" ;;
	...
	# Detect GCE
		...
		case "$GCE_INSTANCE_SIZE" in
			...
			g1-small) su -s/bin/sh consul -c "{ rm /etc/consul/consul.json; jq '.performance.raft_multiplier = 4' > /etc/consul/consul.json; } < /etc/consul/consul.json" ;;
...
if [ -e /data/raft/raft.db ]; then
	...
	if [ "$CONSUL_DC_NAME" ] && [ "$CONSUL_ENCRYPT_TOKEN" ] && [ "$CONSUL_CLUSTER_SIZE" ] && [ "$CONSUL_DNS_NAME" ]; then
		...
		su -s/bin/sh consul -c "{ rm /etc/consul/consul.json; jq '.acl_datacenter = env.CONSUL_ACL_DC | .acl_master_token = env.CONSUL_ACL_MASTER_TOKEN | .acl_agent_master_token = env.CONSUL_ACL_AGENT_MASTER_TOKEN | .acl_agent_token = env.CONSUL_ACL_AGENT_TOKEN | .acl_token = env.CONSUL_ACL_TOKEN' > /etc/consul/consul.json; } < /etc/consul/consul.json"
